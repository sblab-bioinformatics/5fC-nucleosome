...
raw.data<- read.delim("htseq_brain_heart_all.matrix")
...
write.table(rpkm.d, 'rpkm.txt', row.names= FALSE, col.names= TRUE, sep= '\t', quote= FALSE)
...
contrasts<- makeContrasts(
    ...
    brain_vs_heart_ko = tissue_brain.tdg_ko - tissue_heart.tdg_ko,
    levels= design
)
...
for (cntr in colnames(contrasts)){
    ...
    write.table(detable, paste('expr_diff', cntr, 'edgeR.txt', sep= '.'),
