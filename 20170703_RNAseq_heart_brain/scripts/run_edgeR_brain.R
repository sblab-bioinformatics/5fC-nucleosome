...
raw.data<- read.delim("htseq_brain.matrix")
...
write.table(rpkm.d, 'brain_rpkm.txt', row.names= FALSE, col.names= TRUE, sep= '\t', quote= FALSE)
...
for (cntr in colnames(contrasts)){
    ...
    write.table(detable, paste('expr_diff_brain', cntr, 'edgeR.txt', sep= '.'),
